...
export default function ActivitiesExplorer() {
  ...
  useEffect(() => {
    async function load() {
      const res = await fetch("/data/section7_care_management.csv");
    ...
    <div className="relative p-6 space-y-6 bg-gradient-to-b from-slate-50 to-white min-h-screen">
      <div>
        <h1 className="text-3xl font-bold text-slate-800 mb-4">
          Support at Home - Care Management Activities
        </h1>
        <p className="text-sm text-slate-600">
                    Care Management Activities List from Section 7 of{' '}
                    <a
            ...
            <ul className="space-y-4">
              {itemsInCat.map((item, idx) => (
                ...
                >
                  ...
                  {/* Copy button + badge container won't shrink */}
